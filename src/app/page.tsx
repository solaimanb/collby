...
function ChatRoomEntry() {
  ...
  const joinChat = () => {
    ...
    socketRef.current?.emit('join-chat-room', { roomId, username, role });
    // Navigate to chat page with parameters
    window.location.href = `/chat?roomId=${roomId}&username=${username}&role=${role}`;
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md p-6 bg-white/10 backdrop-blur-lg border-none rounded-lg text-white">
        <h1 className="text-2xl font-bold text-center mb-6">Chat Room Entry</h1>
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={joinChat}
            className="w-full bg-blue-500 hover:bg-blue-600 text-white p-2 rounded disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
